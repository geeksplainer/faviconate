...
export function tuplePt(point: Point): [number, number] {
  return [point.x, point.y];
}

export function tupleSz(size: Size): [number, number] {
...
export function scaleToContain(container: Size, scalable: Size): Size {
  const wOriented = makeSz(
    container.width,
    (container.width * scalable.height) / scalable.width
  );
  const hOriented = makeSz(
    (container.height * scalable.width) / scalable.height,
    container.height
  );

  if (
    wOriented.width <= container.width &&
    wOriented.height <= container.height
  ) {
    return wOriented;
  }
...
export class Rectangle {
  ...
  centerAt(point: Point): Rectangle {
    return this.withLocation(
      makePt(point.x - this.width / 2, point.y - this.height / 2)
    );
  }
  ...
  getCardinalPoint(c: CardinalPoint): Point {
    switch (c) {
      ...
      default:
        throw new Error(`Unknown cardinal point ${c}`);
    }
  }
  ...
  withScaledHeight(height: number) {
    return new Rectangle(
      this.left,
      this.top,
  ...
  get isEmpty(): boolean {
    return (
      this.width === 0 && this.height === 0 && this.left === 0 && this.top === 0
    );
  }
