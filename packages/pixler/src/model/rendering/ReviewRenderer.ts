...
import { IconReviewer } from "../IconReviewer";
import { Palette } from "../PaletteService";
import { IconService } from "../IconService";

export function marchingAntsMarker(
...
export class ReviewRenderer {
  constructor(readonly reviewer: IconReviewer, readonly palette?: Palette) {}

  private colorName(color: Color): string {
    if (this.palette) {
      const found = this.palette.colors.find(
        (palColor) => palColor.hex === color.hexRgb
      );
  ...
  private drawCode(
    ...
    const txtBounds = Rectangle.fromSize(txtSize)
      .centerAt(region.center)
      .offset(0, 9);
  ...
  private drawColorCodes(
    ...
    const pixel = makeSz(
      region.width / sample.width,
      region.height / sample.height
    );
  ...
  renderPreview(context: CanvasRenderingContext2D, canvasSize: Size) {
    ...
    const focusSize = makeSz(
      sampleW * pixelSize.width,
      sampleH * pixelSize.height
    );
    const focusLocation = makePt(
      pixelSize.width * this.reviewer.currentHotspot.left,
      pixelSize.height * this.reviewer.currentHotspot.top
    );
  ...
  renderReview(context: CanvasRenderingContext2D, canvasSize: Size) {
    ...
    const cx = context;
    const { sample } = this.reviewer;
    const graphicSize = makeSz(source.width, source.height);

    const contentBounds = Rectangle.fromSize(
      scaleToContain(canvasSize, graphicSize)
